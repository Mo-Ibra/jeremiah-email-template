...
4. `POST https://api.sender.net/v2/message/send` (see [sender-integration.md](./sender-integration.md)).
...
| Unsubscribe/preferences (optional) | Sender-managed, or a mailto/help link |
...
server-rendered template (`components/email/ReviewEmail.tsx` or a plain `.ts` template string).
...
| --- | --- | --- |
| Sent | Return of `/message/send` | `email_sent_at`, `sender_email_id` |
...
Why our own pixel instead of Sender's link tracking for opens: it keeps every event tied to the
review request token and works without Sender's paid webhooks. Sender webhooks can be layered on
later for delivered/bounced.

## 7. Retry / failure handling (MVP)

- If `POST /message/send` fails: mark the review request as created but `email_sent_at` NULL;
